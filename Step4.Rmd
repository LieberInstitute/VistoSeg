...
If you donot have the [tissue_spots_counts.csv]() generated from the command line function [countNuclei]() already, the option to `Get Cell Counts` on the window should be checked. Then you hit the `start` button which opens up the following window.
...
1. Filename with full path for the capture are tif file.

2. Filename with full path for the mat file of the final nuclei segmentation.

3. Filename with full path for the scalefactors json file.

4. Filename with full path for the tissue_positions_list csv file.
