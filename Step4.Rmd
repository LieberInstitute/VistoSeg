...
1. Filename with full path for the `.mat` file of the final nuclei segmentation. Segmentations in `.tif` format from alternative methods can be converted to `.mat` format using the function [tif2mat](https://github.com/LieberInstitute/VistoSeg/blob/main/code/tif2mat.m). We are working on creating functions to convert other formats to `.mat` format.
